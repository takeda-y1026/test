...
int main(int argc, char** argv){
    ros::init(argc, argv, "final_kadai");
    ros::NodeHandle nh;
	...
	while (ros::ok()){	
        ...
        switch (state){
            ...
            case STOP: //一時停止
                ROS_INFO("STOP\n");
                //while (ros::Time::now() - start_time < ros::Duration(3.0)){ //最大３秒間待機
                rateCount = 0;
                while (rateCount < 30){ //最大３秒間待機
                    isObstacle = true;
                    ros::spinOnce();
                    for (int i = center - 15; i < center + 15; i++){
                        if (scan.ranges[i] < 1.0){
            ...
            case AVOID: //障害物回避
                ROS_INFO("AVOID\n");
                if (avoid_step==-1){
                    midPoint1.pose.position.x = robot_x + 1.0;
                    midPoint1.pose.position.y = robot_y + 1.5;
                    midPoint2.pose.position.x = robot_x + 2.0;
                    midPoint2.pose.position.y = robot_y;
                    avoid_step++;
                }
                else if (avoid_step==0){
                    while(ros::ok()){
                        ros::spinOnce();
