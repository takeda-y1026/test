...
int main(int argc, char **argv)
{
    ...
    double constant_angular_speed = 0.5;
    ...
    while (ros::ok())
    {
        if (step == 0)
        {
            ...
            {
                double t = (ros::Time::now() - start_time).toSec();
                twist.linear.x = constant_linear_speed * t;
                twist.angular.z = constant_angular_speed * t;
                twist_pub.publish(twist);
                loop_rate.sleep();
            }
            ...
            while (ros::Time::now() - start_time < ros::Duration(4.0))
            {
                ...
                twist.angular.z = constant_angular_speed;
                twist_pub.publish(twist);
                loop_rate.sleep();
            }
            ...
            {
                double t = (ros::Time::now() - start_time).toSec();
                twist.linear.x = constant_linear_speed * (1.0 - t);
                twist.angular.z = constant_angular_speed * (1.0 - t);
                twist_pub.publish(twist);
                loop_rate.sleep();
            }
